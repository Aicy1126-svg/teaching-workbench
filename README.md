...
> 📘 给**使用者**看的操作指南见同目录 **`使用说明书.md`**（激活、手机安装、各模块用法、数据备份）。本文件是给**分销商 / 部署者**的。
...
### 4. 设置收款二维码（激活页显示）
激活页默认会在输入码上方展示你的收款二维码，引导买家先付款。
- 把你的**微信 / 支付宝收款码**截图，命名为 `payment-qr.png` 放到 `icons/` 目录即可生效；
- 或编辑 `js/activation.config.js` 里的 `PAYMENT_QR` 改成你的图片地址；
- 没放图时激活页会显示「（商家未设置收款码）」占位框，不影响激活功能。
- 付款提示文案改 `MERCHANT_TIP` 字段。

> 激活页样式在 `js/activation.js` 的 `showScreen()`，可自由改排版 / 配色。
